...
[bfx](https://github.com/bmoneill/bfx) is a toolkit for Brainfuck-like languages
written in C99 that includes an interpreter, compiler, and interactive REPL.
Rather than implementing only a basic Brainfuck interpreter, bfx is designed to
support multiple Brainfuck derivatives, including Brainfork, Grin, Pbrain, and
Weave. The project is built with a modular architecture that allows for easy
addition of new language features and optimizations. Support for any esoteric programming
language with a tape-based memory model and single-character instructions can
be added to bfx without modifying the core interpreter logic.
...
- Added support for the following Brainfuck derivatives: Brainfork, Grin,
  Pbrain, and Weave.
- Developed a library runtime that allows for standalone executables that run
  bfx-supported esolangs.
