...
management operations (notably adding `free`) while retaining much of the performance
benefits of region-based allocation.
...
performance or greater flexibility. Bump allocators, while useful for their speed,
have a narrow range of use cases, whereas this allocator is designed to support
a wider variety of workloads, including those that require block reuse, tagging,
and reallocation. See [u225](https://github.com/bmoneill/u225), a video game that
uses Arena for game state management, for an example of a real-world application
that matches Arena's use case.
...
- Supported `malloc`, `calloc`, `realloc`, `memcpy`, and `free` semantics within
  the arena.
- Reusing freed blocks when bookkeeping is enabled to reduce wasted memory.
- Implemented integer-based allocation tags, allowing blocks to be located
  by tag and freed in groups.
...
- Add debugging features such as guard regions, memory poisoning, and corruption
  detection.
